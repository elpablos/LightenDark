...
{this._elementsTreeUpdater=new WebInspector.ElementsTreeUpdater(this._target.domModel,this);},setVisible:function(visible)
...
{RuntimeAgent.releaseObjectGroup(WebInspector.EventListenersSidebarPane._objectGroupName);this._linkifier.reset();var body=this.bodyElement;body.removeChildren();this.sections=[];var self=this;function callback(error,eventListeners)
{if(error)
...
continue;eventListener.node=node.target().domModel.nodeForId(eventListener.nodeId);delete eventListener.nodeId;if(/^function _inspectorCommandLineAPI_logEvent\(/.test(eventListener.handlerBody.toString()))
...
WebInspector.EventListenersSection.prototype={addListener:function(target,eventListener)
{var eventListenerBar=new WebInspector.EventListenerBar(target,eventListener,this._nodeId,this._linkifier);this._eventBars.appendChild(eventListenerBar.element);},__proto__:WebInspector.PropertiesSection.prototype}
WebInspector.EventListenerBar=function(target,eventListener,nodeId,linkifier)
{WebInspector.ObjectPropertiesSection.call(this,target.runtimeModel.createRemoteObjectFromPrimitiveValue(""));this._runtimeModel=target.runtimeModel;this.eventListener=eventListener;this._nodeId=nodeId;this._setNodeTitle();this._setFunctionSubtitle(linkifier);this.editable=false;this.element.className="event-bar";this.headerElement.classList.add("source-code");this.propertiesElement.className="event-properties properties-tree source-code";}
WebInspector.EventListenerBar.prototype={update:function()
{function updateWithNodeObject(nodeObject)
{var properties=[];properties.push(this._runtimeModel.createRemotePropertyFromPrimitiveValue("type",this.eventListener.type));properties.push(this._runtimeModel.createRemotePropertyFromPrimitiveValue("useCapture",this.eventListener.useCapture));properties.push(this._runtimeModel.createRemotePropertyFromPrimitiveValue("isAttribute",this.eventListener.isAttribute));if(nodeObject)
properties.push(new WebInspector.RemoteObjectProperty("node",nodeObject));if(typeof this.eventListener.handler!=="undefined"){var remoteObject=this._runtimeModel.createRemoteObject(this.eventListener.handler);properties.push(new WebInspector.RemoteObjectProperty("handler",remoteObject));}
properties.push(this._runtimeModel.createRemotePropertyFromPrimitiveValue("listenerBody",this.eventListener.handlerBody));if(this.eventListener.sourceName)
properties.push(this._runtimeModel.createRemotePropertyFromPrimitiveValue("sourceName",this.eventListener.sourceName));properties.push(this._runtimeModel.createRemotePropertyFromPrimitiveValue("lineNumber",this.eventListener.location.lineNumber+1));this.updateProperties(properties);}
this.eventListener.node.resolveToObject(WebInspector.EventListenersSidebarPane._objectGroupName,updateWithNodeObject.bind(this));},_setNodeTitle:function()
...
self.node.highlight(self._highlightMode);if(commitEditor){self.dispatchEventToListeners("metrics edited");self.update();}};var allProperties=this.inlineStyle.allProperties;for(var i=0;i<allProperties.length;++i){var property=allProperties[i];if(property.name!==context.styleProperty||property.inactive)
...
this._documentUpdated(treeOutline.target(),treeOutline.domModel().existingDocument());else
...
{delete this.currentQuery;},_documentUpdatedEvent:function(target,event)
{this._documentUpdated(target,(event.data));},_documentUpdated:function(target,inspectedRootDocument)
{this._reset();this.searchCanceled();var treeOutline=this._targetToTreeOutline.get(target);treeOutline.rootDOMNode=inspectedRootDocument;if(!inspectedRootDocument){if(this.isShowing())
target.domModel.requestDocument();return;}
...
var node=nodeId?target.domModel.nodeForId(nodeId):null;selectNode.call(this,node);}
if(this._omitDefaultSelection)
return;if(this._selectedPathOnReset)
target.domModel.pushNodeByPathToFrontend(this._selectedPathOnReset,selectLastSelectedNode.bind(this));else
...
{if(!(treeElement.treeOutline instanceof WebInspector.ElementsTreeOutline)){callback();return;}
...
{remoteObject.pushNodeToFrontend(selectNode.bind(null,(remoteObject.target())));}
var commandCallback;if(object instanceof WebInspector.RemoteObject){var remoteObject=(object);if(remoteObject.subtype==="node")
commandCallback=revealElement.bind(null,remoteObject);}else if(object instanceof WebInspector.DOMNode){var domNode=(object);commandCallback=domNode.reveal.bind(domNode);}
if(!commandCallback)
